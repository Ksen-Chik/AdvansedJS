...
// 1. Пустые классы для корзины товаров и элемента корзины товаров
class BagList {
  // понадобится Метод подсчета количества и стоимости товаров корзины
  // метод добавления товаров на страницу корзины
}
class BagItem {
  // метод добавления товара на страницу корзины
}
class ProductList {
  constructor(container = '.products') {
    this.container = container;
  ...
  // 2. Метод подсчета стоимости товаров
  coastGoods() {
    let result = this._goods.map(item => item.price);
